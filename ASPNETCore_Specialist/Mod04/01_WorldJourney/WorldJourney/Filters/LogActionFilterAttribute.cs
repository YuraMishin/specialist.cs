  ...
  public class LogActionFilterAttribute : ActionFilterAttribute
  {
    ...
    public override void OnActionExecuting(ActionExecutingContext filterContext)
    {
      ...
      using (FileStream fs = new FileStream(_fullPath, FileMode.Create))
      {
        ...
        {
          sw.WriteLine(
            $"The action {actionName} in {controllerName} controller started, event fired: OnActionExecuting");
        }
      }
    }
    ...
    public override void OnActionExecuted(ActionExecutedContext filterContext)
    {
      ...
      using (FileStream fs = new FileStream(_fullPath, FileMode.Append))
      {
        ...
        {
          sw.WriteLine(
            $"The action {actionName} in {controllerName} controller finished, event fired: OnActionExecuted");
        }
      }
    }
    ...
    public override void OnResultExecuted(ResultExecutedContext filterContext)
    {
      ...
      using (FileStream fs = new FileStream(_fullPath, FileMode.Append))
      {
        ...
        {
          sw.WriteLine(
            $"The action {actionName} in {controllerName} controller has the following viewData : {result.ViewData.Values.FirstOrDefault()}, event fired: OnResultExecuted");
        }
      }
    }
  }
